...
FVMesh Mesh::createUnstructuredMesh(
	...
	// Nothing to weld on this path: the triangles already index one shared node
	// list, so the cell corners are the triangle's own vertex IDs.
	mesh.points = points;
	...
	for (int c = 0; c < (int)(triangles.size()); c++) {
		...
		if (tri.v0 < 0 || tri.v1 < 0 || tri.v2 < 0 ||
			...
			continue;	// no outline for this cell -- leaves an empty range
		}

		// CCW for every consumer. gmsh orients its triangles, but the winding is
		// cheap to confirm and a flipped one would invert every angle read off it.
		const double cross =
			(b.z - a.z) * (d.r - a.r) - (b.r - a.r) * (d.z - a.z);

		mesh.cellCornerIDs.push_back(tri.v0);

		if (cross >= 0.0) {
			mesh.cellCornerIDs.push_back(tri.v1);
			mesh.cellCornerIDs.push_back(tri.v2);
		}
		else {
			mesh.cellCornerIDs.push_back(tri.v2);
			mesh.cellCornerIDs.push_back(tri.v1);
	...
	mesh.cellCornerStart[triangles.size()] = (int)(mesh.cellCornerIDs.size());
...
FVMesh Mesh::createMultiBlockFVMesh() const {
	...
	// Cell outlines. The packer carries no vertex IDs at all, so the block node grids
	// are the only record of a structured cell's corners -- and unlike the triangles,
	// they are not a shared node list: neighbouring blocks compute the node on their
	// common seam independently, so it agrees only to rounding. Welded with the key
	// and scale rebuildBoundaryDiscretization uses for boundary vertices.
	std::vector<std::array<Vec2, 4>> quads;

	if (multiBlockCellCorners(quads)) {

		double scale = std::max(g.L, g.R);
		scale = std::max(scale, 1.0);

		const double weldTol = 1e-10 * scale;

		std::unordered_map<PointKey, int, PointKeyHash> pointLookup;

		auto addPoint = [&](const Vec2& p) {
			PointKey key = makePointKey(p, weldTol);
			...
			int pointID = (int)(out.points.size());
		...
		const int nCells = out.numCells();

		out.cellCornerStart.assign(nCells + 1, 0);
		out.cellCornerIDs.reserve((std::size_t)(nCells) * 4);

		std::vector<Vec2> corners;

		for (int c = 0; c < nCells; c++) {

			out.cellCornerStart[c] = (int)(out.cellCornerIDs.size());
			...
			// A block node grid is only CCW while the block itself is not inverted.
			if (polygonSignedArea(corners) < 0.0) {
				std::reverse(corners.begin(), corners.end());
			}

			for (const Vec2& corner : corners) {
				out.cellCornerIDs.push_back(addPoint(corner));
			}
		}
