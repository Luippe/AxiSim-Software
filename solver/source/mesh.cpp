...
const FVMesh& Mesh::refreshFVMesh() {
	...
	if (fvMeshDirty) {
		...
		// Corners are rebuilt from the same source, in the same call, so the cell
		// indexing they share with fvMesh cannot drift.
		buildCellCorners();
		...
		//quality.buildQuality(fvMesh, meshPoints, cellCornerStart, cellCornerIDs);
	}

	return fvMesh;
}

void Mesh::buildCellCorners() {
	...
	// Nodes that ought to coincide are computed independently -- by neighbouring
	// blocks on the multiblock path, by gmsh on the unstructured one -- so they
	// agree only to rounding. Same key and scale rebuildBoundaryDiscretization uses
	// to weld boundary vertices.
	double scale = std::max(g.L, g.R);
	scale = std::max(scale, 1.0);

	const double tol = 1e-10 * scale;

	std::unordered_map<PointKey, int, PointKeyHash> pointLookup;

	auto addPoint = [&](const Vec2& p) {
		PointKey key = makePointKey(p, tol);

		auto it = pointLookup.find(key);

		if (it != pointLookup.end()) {
			return it->second;
		}

		int pointID = (int)(meshPoints.size());

		meshPoints.push_back(p);
		pointLookup.emplace(key, pointID);

		return pointID;
	};

	// Reused rather than re-derived: the corner quads are already the canonical
	// outline for the structured path, and the inspector builds the same list.
	std::vector<std::array<Vec2, 4>> quads;

	const bool structured = (currentMeshType == MeshType::Structured);
	const bool haveQuads = structured && multiBlockCellCorners(quads);

	cellCornerStart.assign(nCells + 1, 0);
	cellCornerIDs.reserve((std::size_t)(nCells) * (structured ? 4 : 3));

	std::vector<Vec2> corners;

	for (int c = 0; c < nCells; c++) {

		cellCornerStart[c] = (int)(cellCornerIDs.size());

		corners.clear();

		if (structured) {
			if (haveQuads && c < (int)(quads.size())) {
				corners.assign(quads[c].begin(), quads[c].end());
			}
		}
		else if (c < (int)(unstructuredTriangles.size())) {
			// createUnstructuredMesh resizes cells to triangles.size() and fills
			// them in order, so cell c IS triangle c.
			const Triangle& tri = unstructuredTriangles[c];

			const int nPoints = (int)(unstructuredPoints.size());

			if (tri.v0 >= 0 && tri.v1 >= 0 && tri.v2 >= 0 &&
				tri.v0 < nPoints && tri.v1 < nPoints && tri.v2 < nPoints) {

				corners = {
					unstructuredPoints[tri.v0],
					unstructuredPoints[tri.v1],
					unstructuredPoints[tri.v2]
				};
			}
		}

		// Anything short of a polygon is left as an empty range -- a partial outline
		// would silently pick and draw as a degenerate cell.
		if (corners.size() < 3) {
			continue;
		}

		// One winding for every consumer. gmsh orients its triangles, but a block
		// node grid is only CCW while the block itself is not inverted.
		if (polygonSignedArea(corners) < 0.0) {
			std::reverse(corners.begin(), corners.end());
		}

		for (const Vec2& corner : corners) {
			cellCornerIDs.push_back(addPoint(corner));
		}
	}

	cellCornerStart[nCells] = (int)(cellCornerIDs.size());
}
